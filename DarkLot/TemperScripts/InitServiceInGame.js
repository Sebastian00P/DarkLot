...
(function () {
        ...
        if (window.Engine && Engine.communication && !Engine.communication._dl_patchedTeam) {
            ...
            Engine.communication.successData = function (jsonStr) {
                ...
                if (parsed.f && parsed.f.w) {
                    ...
                    console.group('[DL][DEBUG] parsed.f.w'); console.log(parsed.f.w); console.groupEnd();
                }
        ...
        if (window.Engine && Engine.battle && !Engine.battle._dl_patchedTurn) {
            ...
            Engine.battle.updateData = function (data) {
                ...
                if (this.warriorsList && Object.keys(this.warriorsList).length) {
                    ...
                    console.group('[DL][DEBUG] warriorsList'); console.log(window._dl_lastWarriorsListTurn); console.groupEnd();
                }
                return res;
            };
    ...
    const patchInterval = setInterval(() => {
        ...
        if (Engine.communication._dl_patchedTeam && Engine.battle._dl_patchedTurn) clearInterval(patchInterval);
    }, 100);
    ...
    function buildDto(items) {
        console.log('[DL][DEBUG] buildDto items=', items);
        const mobParts = [];
        const lootUsers = [];
        const fastNames = window._dl_teamParsedFW || {};
        const turnStats = window._dl_lastWarriorsListTurn || {};
        // Priorytet: turn-based gdy są dodatnie ID w turnStats
        ...
        console.log('[DL][DEBUG] DTO:', dto);
        return dto;
    }

    // WYSYŁKA LOOT
    function sendCachedLoot() {
        if (!sessionChecked) return console.log('[DL][DEBUG] session not checked');
        if (!window._dl_lastLootEventId) return console.log('[DL][DEBUG] no lastLootEventId');
        if (sentLootEvents.has(window._dl_lastLootEventId)) return console.log('[DL][DEBUG] already sent');

        const lootCache = window._dl_lootCache[window._dl_lastLootEventId];
        if (!lootCache) return console.log('[DL][DEBUG] missing cache entry');
        ...
        if (!dto.lootUsers.length || !dto.mobName || !dto.items.length) {
            console.log('[DL][DEBUG] skipping send: insufficient data');
    ...
    const btn = document.createElement('button'); btn.id = 'dl-lootlog-btn'; btn.innerHTML = '<b style="font-size:22px;letter-spacing:2px;">DL</b>';
    ...
    function showDLWindow() {
        ...
        const win = document.createElement('div'); win.id = 'dl-lootlog-window'; win.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#181c22;color:white;border:2.5px solid #18a8ff;border-radius:18px;z-index:10001;padding:38px 28px 24px 28px;box-shadow:0 4px 32px #000b;';
        win.innerHTML = '<div style="display:flex;align-items:center;gap:16px;"><span style="font-size:30px;width:38px;height:38px;border-radius:50%;background:#23272e;border:2.5px solid #18a8ff;text-align:center;line-height:36px;box-shadow:0 0 16px #31c6ff55;"><b style="font-size:22px;letter-spacing:2px;color:#18a8ff;">DL</b></span><span style="font-size:20px;">Lootlog</span></div><div id="dl-status" style="margin:18px 0;">Sprawdzanie sesji...</div><button id="dl-session-btn" style="display:none;margin-right:10px;padding:7px 18px;font-size:16px;background:#0056d6;color:white;border:none;border-radius:7px;cursor:pointer;">Sprawdź sesję</button><button id="dl-close-btn" style="position:absolute;top:10px;right:18px;font-size:18px;background:none;color:#bbb;border:none;cursor:pointer;">×</button>';
        document.body.appendChild(win);
        win.querySelector('#dl-close-btn').onclick = () => win.remove(); win.querySelector('#dl-session-btn').onclick = checkSession; checkSession();
    }
    ...
    const observer = new MutationObserver(muts => muts.forEach(m => m.addedNodes.forEach(node => { if (node.nodeType === 1 && node.classList.contains('loot-wnd')) setTimeout(sendCachedLoot, 100); }))); observer.observe(document.body, { childList: true, subtree: true });

    // WYSYŁKA do serwera
    function sendLootToServer(dto) { fetch('https://localhost:7238/api/lootlog/add', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(dto) }).then(r => r.json()).then(res => { if (res.status === 'ok') console.log('[DL] Loot sent'); else console.error('[DL] Error', res); }).catch(err => console.error('[DL] Conn error', err)); }

    // Reset cache
    function resetDLCache() { window._dl_teamParsedFW = null; window._dl_lastWarriorsListTurn = null; window._dl_lastLootEventId = null; window._dl_lootCache = {}; window._dl_sentItemHids = new Set(); }
})();
